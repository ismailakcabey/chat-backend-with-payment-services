...
export class CompanyService implements ICompany{
    ...
    async createCompany(createCompany: CreateCompanyDto): Promise<{ status?: boolean; data?: Company; message?: string; messageType?: number; }> {
        try {
    ...
    async updateCompany(id: string, updateCompany: UpdateCompanyDto): Promise<{ status?: boolean; data?: Company; message?: string; messageType?: number; }> {
        const company = await this.companyModel.findByIdAndUpdate(id, updateCompany)
        if(company){
            return{
                status:true,
                data:company,
    ...
    async deleteCompany(id: string): Promise<{ status?: boolean; data?: Company; message?: string; messageType?: number; }> {
    ...
    async findByIdCompany(id: string): Promise<{ status?: boolean; data?: Company; message?: string; messageType?: number; }> {
        try {
